...
impl Render for TextProbe {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .child(div().font_family("Helvetica").child("Helvetica: GPUI text 0123456789"))
            .child(div().font_family(".SystemUIFont").child("System: GPUI text 0123456789"))
            .child(div().font_family("Menlo").child("Menlo: GPUI text 0123456789"))
            .child(
                div().p_4().bg(rgb(0x182033)).text_color(rgb(0xffffff))
                    .font_family("Helvetica").child("White Helvetica on dark background"),
            )
                            ...
                            if let Some(path) = std::env::var_os("GPUI_TEXT_PROBE_SNAPSHOT") {
                                std::thread::spawn(move || {
                                    ...
                                    let result = std::process::Command::new("/usr/sbin/screencapture")
                                        ...
                                        .arg(&path).output();
                                    match result {
                                        Ok(output) => eprintln!("GPUI_TEXT_PROBE capture={} path={} stderr={}",
                                            output.status, std::path::Path::new(&path).display(),
                                            String::from_utf8_lossy(&output.stderr)),
                                        Err(error) => eprintln!("GPUI_TEXT_PROBE capture failed: {error}"),
                                    }
                                });
                            }
                        }
