// Native SubTake capture helper, adapted from the AGPL-licensed Recordly helper.
import Foundation
...
final class ScreenCaptureRecorder: NSObject, SCStreamOutput, SCStreamDelegate {
	...
	private let captureDiagnostics = ProcessInfo.processInfo.environment["SUBTAKE_CAPTURE_DIAGNOSTICS"] == "1"
	private var captureDiagnosticCount = 0
	...
	func startCapture(configJSON: String) async throws {
		...
		if let windowId = config.windowId {
			...
			// Include only the selected window, never other apps covering its bounds.
		...
		// A static window may emit its only complete frame during startCapture.
	...
	func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of outputType: SCStreamOutputType) {
		...
		if outputType == .screen {
			...
			if captureDiagnostics, frameCount == 0, captureDiagnosticCount < 8 {
				captureDiagnosticCount += 1
				let info = CMSampleBufferGetSampleAttachmentsArray(sampleBuffer, createIfNecessary: false)
				fputs("FIRST_FRAME image=\(CMSampleBufferGetImageBuffer(sampleBuffer) != nil) writer=\(assetWriter?.status.rawValue ?? -1) ready=\(videoInput?.isReadyForMoreMediaData ?? false) info=\(String(describing: info))\n", stderr)
			}
