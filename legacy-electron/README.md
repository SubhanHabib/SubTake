> **Legacy Electron/React application.** The current Rust/Slint SubTake app is at the [repository root](../README.md). Run the commands below from `legacy-electron/`. Original GitHub workflows are preserved in `.github/` here as reference and are not active root workflows.
