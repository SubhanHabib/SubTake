...
| Timeline | Trim union, clips, speed, split, delete, both-edge resize, group selection/movement, copy/cut/paste/duplicate with clip audio settings, markers, zoom/pan, ruler, snapping, thumbnails and waveform | Mapping/editing tests and native UI smoke; full interaction parity with the existing timeline is not established.. A hidden lane leaves the preview and export, a muted audio lane plays silence, and the project saves the state |
...
5. Complete VFR/rotation/HDR/long-GOP/long-session/media-loss validation and sustained playback synchronization. The in-process FFmpeg decoder seeks by stream timestamps, but the shared request clock still quantizes to the probed frame rate; it uses software decoding with RGBA upload/readback, not a zero-copy VideoToolbox/D3D pipeline. A take larger than 1920 px on its longest edge gets a background preview proxy (H.264 at 1920, a keyframe every quarter second, its own timestamps) that playback decodes from; a paused frame and the export still read the take. A synthetic 3456×2234 take decodes at 6.5 ms a frame and its proxy at 2.4 ms. Until the proxy is made, playback decodes the take at full size, since scaling it down in the decoder measured slower (10.9 ms). The proxy is not yet tested on long, VFR or HDR takes.
