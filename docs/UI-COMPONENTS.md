...
| Sliders | `slider.rs` | The scrub field — a filled slider that *is* the row, with its caption inside the plate and its value in Geist Mono on the right. Track `sunk`, hover `sunk2`; the fill is a flat `slider_fill` from the left end, square at its right edge, with no tick. A 4-wide thumb sits on the fill edge only on hover (`muted`) and while dragging (`text`, taller), when the value turns `text` at 500. The track's rounded left end is a track-sized pill cut to the level's width, since gpui clips to rectangles. Drag previews, release commits, and the committed number is quantised to the step the display shows. |
| Switches | `switch.rs` | `toggle` and `switch`: `switch_off` off, `switch_on` on, a white thumb in both states travelling 18 in 140ms. Not wired: the thumb widening to 26 while held. |
